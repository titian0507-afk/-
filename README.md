...
个人作品集：[在线预览](https://luohao-studio.site/)。作品分为 **访谈 / 短视频 / 平面与H5**，共 29 个项目。
...
网站从 `main` 分支由腾讯云 EdgeOne Pages 自动构建并发布，构建命令是 `npm run build`，产物目录为 `dist`。原视频和 PSD 工作文件不在仓库中；网页内使用的是从用户提供的本地作品集制作的适配版素材。`assets/hero-collage.png` 是为网页生成的黑白胶片风格背景。
